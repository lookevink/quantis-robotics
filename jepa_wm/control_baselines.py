...
from jepa_wm.control_rollout import ControlRolloutReport, PoseError
...
class RealizedBaselineComparison:
    ...
    def direct_improves_over_zero(self) -> PoseAxisDecision:
        return PoseAxisDecision(
            translation=(
                self.direct.translation_progress_meters
                > self.zero.translation_progress_meters
            ),
            rotation=(
                self.direct.rotation_progress_radians
                > self.zero.rotation_progress_radians
            ),
            gripper=(
                self.direct.gripper_progress > self.zero.gripper_progress
            ),
        )
    ...
    def direct_reaches_scripted_tolerance(self) -> PoseAxisDecision:
        return PoseAxisDecision(
            translation=(
                self.direct.final_error.translation_meters
                <= self.scripted.final_error.translation_meters
                + self.tolerances.maximum_translation_difference_meters
            ),
            rotation=(
                self.direct.final_error.rotation_radians
                <= self.scripted.final_error.rotation_radians
                + self.tolerances.maximum_rotation_difference_radians
            ),
            gripper=(
                self.direct.final_error.gripper_closedness
                <= self.scripted.final_error.gripper_closedness
                + self.tolerances.maximum_gripper_difference
            ),
        )
    ...
    def from_rollouts(
        ...
        *,
        ...
        for report in reports[1:]:
            reset_error = PoseError.between(direct_initial, cls._initial_pose(report))
            state = report.complete_steps[0].state
            if (
                reset_error.translation_meters
                > reset_tolerances.maximum_translation_difference_meters
                or reset_error.rotation_radians
                > reset_tolerances.maximum_rotation_difference_radians
                or reset_error.gripper_closedness
                > reset_tolerances.maximum_gripper_difference
                or not np.allclose(
                    state.current_joint_positions,
                    direct_state.current_joint_positions,
                    rtol=0.0,
                    atol=reset_tolerances.maximum_joint_difference_radians,
                )
            ):
                raise ValueError("baseline trials did not start from the same reset")
        if any(
            step.state.collision_detected
            or step.state.contact_force_newtons
            > reset_tolerances.maximum_reset_contact_force_newtons
            for report in reports
            for step in report.complete_steps[:1]
        ):
            raise ValueError("baseline trial reset contains collision or contact")
