...
case "${command}" in
  ...
  jepa-wm-unknown-start-acquisition-hold)
    source_revision="$(deployment_source_revision)"
    runtime_fingerprint="$(python3 -m jepa_wm.contact_grasp_acquisition_hold fingerprint)"
    command_status=0
    sync_repo || command_status=$?
    if (( command_status == 0 )); then
      remote "bash ~/quantis-robotics/ops/run_unknown_start_acquisition_hold.sh '${source_revision}' '${runtime_fingerprint}'" \
        || command_status=$?
    fi
    backup_status=0
    remote_with_config 'bash ~/quantis-robotics/ops/backup_state.sh' || backup_status=$?
    if (( command_status == 0 && backup_status == 0 )); then
      remote "cd ~/quantis-robotics && ~/.venvs/quantis-jepa-wm/bin/python -m jepa_wm.contact_grasp_acquisition_hold finalize --checkpoint-root ~/docker/jepa-wm/checkpoints --recovery-checkpoint-root /mnt/quantis-assets/quantis-state/jepa-wm/checkpoints --data-root ~/docker/isaac-sim/data/quantis --recovery-data-root /mnt/quantis-assets/quantis-state/isaac" \
        || command_status=$?
      if (( command_status != 0 )); then
        remote "cd ~/quantis-robotics && ~/.venvs/quantis-jepa-wm/bin/python -m jepa_wm.contact_grasp_acquisition_hold failure --checkpoint-root ~/docker/jepa-wm/checkpoints --error 'recovery_finalization:exit_${command_status}'" || true
        remote_with_config 'bash ~/quantis-robotics/ops/backup_state.sh' || true
      fi
    elif (( command_status == 0 )); then
      command_status=${backup_status}
      remote "cd ~/quantis-robotics && ~/.venvs/quantis-jepa-wm/bin/python -m jepa_wm.contact_grasp_acquisition_hold failure --checkpoint-root ~/docker/jepa-wm/checkpoints --error 'recovery_backup:exit_${backup_status}'" || true
    fi
    printf 'Unknown-start acquisition hold workflow complete.\n'
    exit "${command_status}"
    ;;
  jepa-wm-unknown-start-acquisition-resolution)
    source_revision="$(deployment_source_revision)"
    runtime_fingerprint="$(python3 -m jepa_wm.contact_grasp_acquisition_resolution fingerprint)"
    command_status=0
    sync_repo || command_status=$?
    if (( command_status == 0 )); then
      remote "bash ~/quantis-robotics/ops/run_unknown_start_acquisition_resolution.sh '${source_revision}' '${runtime_fingerprint}'" \
        || command_status=$?
    fi
    backup_status=0
    remote_with_config 'bash ~/quantis-robotics/ops/backup_state.sh' || backup_status=$?
    if (( command_status == 0 && backup_status == 0 )); then
      remote "cd ~/quantis-robotics && ~/.venvs/quantis-jepa-wm/bin/python -m jepa_wm.contact_grasp_acquisition_resolution finalize --checkpoint-root ~/docker/jepa-wm/checkpoints --recovery-checkpoint-root /mnt/quantis-assets/quantis-state/jepa-wm/checkpoints --data-root ~/docker/isaac-sim/data/quantis --recovery-data-root /mnt/quantis-assets/quantis-state/isaac" \
        || command_status=$?
      if (( command_status != 0 )); then
        remote "cd ~/quantis-robotics && ~/.venvs/quantis-jepa-wm/bin/python -m jepa_wm.contact_grasp_acquisition_resolution failure --checkpoint-root ~/docker/jepa-wm/checkpoints --error 'recovery_finalization:exit_${command_status}'" || true
        remote_with_config 'bash ~/quantis-robotics/ops/backup_state.sh' || true
      fi
    elif (( command_status == 0 )); then
      command_status=${backup_status}
  ...
  jepa-wm-contact-grasp-horizon-completion)
    ...
    if (( command_status == 0 )); then
      demo_python \
        "demo.diagnose_contact_grasp_followup_drive_target('unknown-start-e2e-v27-62605-grasp-020')" \
        120 || command_status=$?
    fi
    if (( command_status == 0 )); then
