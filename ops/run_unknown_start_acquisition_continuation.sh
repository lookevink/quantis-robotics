...
run_id="unknown-start-e2e-v6-62605-grasp"
source_session="unknown-start-e2e-v5-62605-grasp-23"
...
module="jepa_wm.contact_grasp_acquisition_continuation"
...
[[ "${status}" == "applied" && "${grasp_ready}" == "true" ]] || {
  printf 'error: continuation did not reach retained grasp\n' >&2
  false
}

phase="evaluation"
"${python_bin}" -m "${module}" evaluate \
