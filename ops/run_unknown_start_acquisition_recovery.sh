...
run_id="unknown-start-e2e-v5-62605-grasp"
source_session="unknown-start-e2e-v4-62605-grasp-03"
...
cd "${repo_dir}"
phase="claim"
"${python_bin}" -m jepa_wm.contact_grasp_acquisition_handoff claim \
  --checkpoint-root "${checkpoint_root}" \
  --data-root "${data_root}" \
  --followup-session "${first_session}" \
...
terminalize_failure() {
  ...
  "${python_bin}" -m jepa_wm.contact_grasp_acquisition_handoff failure \
    --checkpoint-root "${checkpoint_root}" \
...
encoded_handoff="$(${python_bin} -m jepa_wm.contact_grasp_acquisition_handoff \
  encode --checkpoint-root "${checkpoint_root}")"
phase="worker_start"
...
for ((index=1; index<=maximum_actions; index++)); do
  ...
  grasp_status="$(${python_bin} -m jepa_wm.control_rollout_cli \
    ...
    --predecessor-session "${source_session}")"
...
[[ "${status}" == "applied" && "${grasp_ready}" == "true" ]] || {
  printf 'error: recovered acquisition did not reach retained grasp\n' >&2
  false
}

phase="evaluation"
"${python_bin}" -m jepa_wm.contact_grasp_acquisition_handoff evaluate \
