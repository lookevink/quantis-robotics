...
class AwsLifecycleTests(unittest.TestCase):
    ...
    def test_unknown_start_acquisition_continuation_is_recovery_gated(self):
        result, calls = self.run_command(
            "jepa-wm-unknown-start-acquisition-continuation"
        )

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("run_unknown_start_acquisition_continuation.sh", calls)
        self.assertIn("ops/backup_state.sh", calls)
        self.assertIn("contact_grasp_acquisition_continuation finalize", calls)

    def test_unknown_start_acquisition_continuation_terminalizes_backup_failure(
        self,
    ):
        result, calls = self.run_command(
            "jepa-wm-unknown-start-acquisition-continuation",
    ...
    def test_unknown_start_acquisition_resolution_is_recovery_gated(self):
        result, calls = self.run_command(
            "jepa-wm-unknown-start-acquisition-resolution"
        )

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("run_unknown_start_acquisition_resolution.sh", calls)
        self.assertIn("ops/backup_state.sh", calls)
    ...
    def test_contact_grasp_blocked_ik_tolerance_diagnostic_is_no_actuation(self):
        result, calls = self.run_command(
            "jepa-wm-contact-grasp-blocked-ik-tolerance-diagnostic",
            arguments=("grasp-to-insertion-run-grasp-01",),
        )

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("diagnose_contact_grasp_blocked_ik_tolerances", calls)
        self.assertIn("grasp-to-insertion-run-grasp-01", calls)
        self.assertNotIn("backup_state.sh", calls)
        self.assertNotIn("run_control_step.sh", calls)

    def test_contact_grasp_active_rotation_ik_diagnostic_is_no_actuation(self):
        result, calls = self.run_command(
            "jepa-wm-contact-grasp-active-rotation-ik-diagnostic",
            arguments=("grasp-to-insertion-run-grasp-15",),
        )

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("diagnose_contact_grasp_active_rotation_ik", calls)
        self.assertNotIn("backup_state.sh", calls)
        self.assertNotIn("run_control_step.sh", calls)
